...
$$\displaystyle J(w, b) = \frac{1}{m} \sum_{i=1}^{m}{L(\hat{y}^{(i)}, y^{(i)})} + \frac{\lambda}{2m}{\left\| w \right\|}_{2}^{2}$$

위 식은 $L_{2}$ 정규화이다. ${\left\| w \right\|}_{2}^{2}$ 는 다음과 같이 구한다.
...
$$\displaystyle J(w^{[1]}, b^{[1]}, ..., w^{[L]}, b^{[L]}) = \frac{1}{m} \sum_{i=1}^{m}{L(\hat{y}^{(i)}, y^{(i)})} + \frac{\lambda}{2m}\sum_{l=1}^{L}{{\left\| w^{[l]} \right\|}_{F}^{2}}$$

식은 매우 유사하다. 다만, ${\left\| w^{[l]} \right\|}_{F}^{2}$ 가 마음에 걸릴 수 있는데, 다음과 같이 계산한다.

$${\left\| w^{[l]} \right\|}_{F}^{2} = \sum_{i=1}^{n^{[l]}}\sum_{i=1}^{n^{[l-1]}}{(w_{ij}^{[l]})^2}$$
...
$$\displaystyle J(w^{[l]}, b^{[l]}) = \frac{1}{m} \sum_{i=1}^{m}{L(\hat{y}^{(i)}, y^{(i)})} + \frac{\lambda}{2m}\sum_{l=1}^{L}{{\left\| w^{[l]} \right\|}_{F}^{2}}$$
