...
- Current input vector $\mathbf{x}_{(t)}$, and short-term state of the previous step $\mathbf{h}_{(t-1)}$

    Injected to 4 different fully connected layers.
        
    - One main layer returns $\mathbf{g}_{(t)}$. The output will go to sum calculation of $\mathbf{c}_{(t-1)}$.
...
- No output gate. Therefore, whole state vector is returned for each time step. There are a new gate controller $\mathbf{r}_{(t)}$ to regulate which part will be revealed to the main layer $\mathbf{g}_{(t)}$.
